...
# Global counter for backward_general calls (internal backward passes)
_BACKWARD_GENERAL_CALL_COUNT = 0

def get_backward_general_call_count():
    return _BACKWARD_GENERAL_CALL_COUNT

def reset_backward_general_call_count():
    global _BACKWARD_GENERAL_CALL_COUNT
    _BACKWARD_GENERAL_CALL_COUNT = 0
...
def backward_general(
    ...
    # Increment global backward_general counter (always print for debugging)
    global _BACKWARD_GENERAL_CALL_COUNT
    _BACKWARD_GENERAL_CALL_COUNT += 1
    print(f"[AUTO_LIRPA COUNTER] backward_general call #{_BACKWARD_GENERAL_CALL_COUNT}: "
          f"bound_node={bound_node.name}")
